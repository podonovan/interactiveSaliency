...
function setupCanvas() {
	...
	/*
	 $('#canvas').data("saved_layouts",[]);
	 $('#canvas').data("saved_images",[]);
	 $('#canvas').data("saved_designs",[]);

	 $('#canvas').data("gallery_layouts",[]);
	 $('#canvas').data("gallery_images",[]);
	 $('#canvas').data("gallery_designs",[]);

	 $("#canvas").data("max_gallery_size",100)
	 */
...
//
// 	//if ($('#canvas').data("noSuggestions"))
// 	//	return console.log("resumeSuggestions")
//
// 	console.log("sendCurrentLayout")
//
...
// 	//sendCurrentLayout();
//
// 	$('#canvas').data("user_input_log").push("Load-" + layout_type)
// 	$('#canvas').data("status_log").push("Load-" + layout_type)
//
// 	//if (!$('#canvas').data("noSuggestions"))
// 	//	$('#suggestion_lk').click()
//
...
//
//
// function toggleAverageSaliency(){
// 	console.log("toggleAverageSaliency")
//
// 	if ($('#canvas').data("orig_importance_select")){
// 		$('#canvas').data("orig_importance_select",false)
// 	}
// 	else{
// 		$('#canvas').data("orig_importance_select",true)
// 	}
//
// 	$('#canvas').data("saliency_layout",'')
// }
...
// 	var run_types = ['near']
//
// 	if (getURLParameter("styles") == 1)
// 		run_types.push("gallery")
//
...
//
// 		startNewRun(run.id, run_types[i], design_string)
// 		runs.push(run)
//
...
// 	}
//
// 	$('#canvas').data("runs", runs)
//
// 	setTimeout(sendCurrentLayout, 500);
//
// }
...
//
// 	if ((run.layout != new_layout) && ((run.energy != new_energy ) || (run.type.search("gallery") > -1))) {
//
...
//
// 						var canvas = $('#suggestion_layout'+(run.canvas_id))[0];
// 						var ctx = canvas.getContext("2d")
//
...
// 	//$('#pauseButton').removeClass("btn-success")
// 	$('#pauseButton').show()
// 	$('#startButton').hide()
// 	$('#canvas').data("paused", false)
//
...
// }
//
// function checkElementImages(elem) {
//
// 	var design=$('#canvas').data("design");
// 	var layer = $('#canvas').data("stage").get('#layer')[0];
//
...
// 				//if (hitId != undefined)
// 				//{
// 				var fname = 'layouts/' + $('#canvas').data("design").name + '-' + gup('workerId') + "-" + hitId + "-" + String(sugg_id) + '.png'
...
// 		$('#canvas').data("user_input_log").push("State-tweakable")
// 		$('#canvas').data("status_log").push("State-tweakable")
//
// 	});
//
...
//
// 		$('#canvas').data("user_input_log").push("State-locked" )
// 		$('#canvas').data("status_log").push("State-locked" )
// 	});
// 	elem.lock_img.on("click tap", function(evt) {
// 		setElementState(elem.id, 'unlocked')
//
// 		$('#canvas').data("user_input_log").push("State-unlocked")
// 		$('#canvas').data("status_log").push("State-unlocked")
// 	});
//
// }
//
// function setElementState(elem_id, new_state) {
// 	$.each($('#canvas').data("design").elements, function(i, elem) {
//
// 		if (elem.id == elem_id) {
//
...
//
// function setCurrentLayout(new_layout, set_fixed, undoing) {
//
...
//
// 	if (($('#canvas').data("paused") && (set_fixed == false)) || ($('#canvas').data("dragging")))
// 		return
//
// 	var design = $('#canvas').data("design")
...
//
// 		if (!undoing) {
// 			addLayoutToUndoStack(new_layout)
//
// 		}
...
// 		{
//
// 		var n=String(children[i].attrs.name)
//
// 		if ((n.indexOf("top") == -1) && (n.indexOf("bottom") == -1))
// 		console.log(i+": "+children[i].attrs.name)
//
// 		if (!((n.indexOf("background")) || (n.indexOf("preview") == -1)))
// 		children[i].remove()
//
// 		}
// 		*/
...
// 			//elem.tweakable_img.hide()
//
// 			//bool visible=elem.state_img.attrs.visible
//
// 			var alt_id = parseInt(elem_split[6]);
//
// 			if ((!elem.fix_alternate) && (elem.id in design.element_alts)) {
// 				//elem.alternate_id=alt_id
// 				$.each(design.element_alts[elem.id], function(num_lines, ae) {
// 					ae.img.remove();
// 					if (alt_id == ae.alternate_id) {
// 						ae.fixed_amount = elem.fixed_amount
// 						//ae.state_img = elem.state_img
// 						design.elements[i - 3] = ae
// 						elem = ae;
// 					}
// 				});
// 			}
...
// 			layer.add(elem.img)
//
// 			elem.height = parseInt(elem_split[2]);
// 			elem.width = elem.height * elem.aspect_ratio;
// 			elem.x = parseInt(elem_split[0])
// 			elem.y = parseInt(elem_split[1])
// 			//elem.state_img.setPosition(elem.x + elem.width, elem.y)
// 			//elem.state_img.show()
//
//
...
//
// 			//console.log(elem_split)
// 			//console.log("setting to "+elem.x+" "+elem.y+" "+elem.height)
//
// 			if (elem.num_lines > 1) {
//
// 				var alt = parseInt(elem_split[3])
// 				elem.align_type = alt
//
// 				var align;
// 				if (alt > -1)
// 					align = alignments[alt]
// 				else
// 					align = elem.align
//
...
// 			}
//
// 			elem.img.opacity = elem.opacity;
// 			elem.img.setX(elem.x);
// 			elem.img.setY(elem.y);
// 			elem.img.setHeight(elem.height);
// 			elem.img.setWidth(elem.width);
// 			moveAnchors(elem)
//
// 			setHidden(elem, alt_id < 0)
//
// 		}
//
// 		// //set regions
// 		// design.regions = []
// 		// for (var i = elem_cnt + 3; i < elements.length; i++) {
...
//                    var hitId = gup('hitId')
//                    var fname = 'layouts/' + $('#canvas').data("design").name + '-' + gup('workerId') + "-" + hitId + "-" + String(sugg_id) + '.png'
//                    requestSaliencyImage(fname, dataUrl,function(saliencyURL) {
//                        console.log("saliency url:"+saliencyURL)
//
//                         })
//
...
// 	var curr_layout = getCurrentLayout()
//
// 	$('#canvas').data("ignore_layout", curr_layout)
//
// 	console.log("resetting from layout:" + $('#canvas').data("ignore_layout"))
//
// 	$.each(design.elements, function(i, e) {
//
// 		var new_height = 0
...
// 			w = elem.width
// 			h = elem.height
// 			elements += Math.round(elem.x) + "," + Math.round(elem.y) + "," + Math.round(w)+"," + Math.round(h)+ "\n"
// 		}
// 	}
// 	return elements;
//
...
// 		if (run.id > -1)
// 			getLayoutFromServer(run.id, run.type);
// 	});
// 	/*
